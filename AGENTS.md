...
Enrichment is **incremental**: it loads the committed `classifications.csv` and reuses a repo's categories while nothing the classifier reads has changed, so re-runs only pay the LLM cost for new or updated repos. The reuse key is `classifier_signature` alone — a hash of the whole prompt: the GitHub description, the topics and the README as truncated for the prompt, plus `CATEGORY_TREE_TEXT` and `CLASSIFIER_INSTRUCTIONS`, stored in the `signals` column. It replaced `pushed_at`, which failed in both directions: it only moves on a commit, so a description or topic list edited in the repository settings kept a stale classification indefinitely (and for a repo with no README those two are the *only* inputs); and it moves on *every* commit, so a code-only push — most pushes — paid for an answer that could not have changed. That second half cost 188 of 533 repos in the 2026-09 run. The signature is the exact fingerprint of what the model is shown, so it is both stricter and cheaper than the date. The taxonomy and the contract are hashed with the repo because they are as much a part of the prompt as the README, and since a push no longer invalidates anything, nothing else would ever re-ask — editing either re-classifies the whole listing, which is exactly when every cached answer is stale. The backend and its model are deliberately **not** hashed: they are who answers, not what was asked, and keying on them would make local `claude` runs and CI `copilot` runs thrash each other's cache. `enrich` exits non-zero when every attempt it made failed — a retired model id or an unauthenticated CLI fails every repo identically, each one keeps its cached category, and the regression guard downstream would see nothing wrong and open a PR claiming a refresh that never happened. One flaky repo among several is not that, and stays a warning. A cache entry written before the column existed carries no signature and is still reused; every entry is stamped as it is rewritten, reused ones included, so the check is live for every repo from the next run on. A failed classification leaves that repo's cache entry unchanged, so the next run retries it on its own; `--refresh` ignores the cache and re-classifies every repo.
